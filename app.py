...
def main():
    ...
    loaded_file = st.file_uploader(f"Load your file {option.split()[0]}", type=[option.split()[0].lower()])
    ...
    if loaded_file is not None:
        try:
            if option == "Parquet to CSV":
                ...
                df = pd.read_csv(loaded_file, sep=option_2)
        ...
        st.warning(f"Please, load a file {option.split()[0]}.")
